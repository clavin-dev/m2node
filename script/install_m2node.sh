...
install_v2node() {
    ...
    if [[ -e /usr/local/m2node/ ]]; then
        rm -rf /usr/local/m2node/
    fi

    mkdir /usr/local/m2node/ -p
    cd /usr/local/m2node/
    ...
    if  [[ -z "$version_param" ]] ; then
        ...
        url="https://github.com/clavin-dev/m2node/releases/download/${last_version}/v2node-linux-${arch}.zip"
        curl -sL "$url" | pv -s 30M -W -N "下载进度" > /usr/local/m2node/v2node-linux.zip
        if [[ $? -ne 0 ]]; then
    ...
    chmod +x v2node
    mkdir /etc/m2node/ -p
    cp geoip.dat /etc/m2node/
    cp geosite.dat /etc/m2node/
    if [[ x"${release}" == x"alpine" ]]; then
...
command="/usr/local/m2node/v2node"
command_args="server"
command_user="root"
...
EOF
        ...
        cat <<EOF > /etc/systemd/system/m2node.service
[Unit]
Description=m2node Service (ShadowFlow)
...
LimitCORE=infinity
LimitNOFILE=999999
WorkingDirectory=/usr/local/m2node/
ExecStart=/usr/local/m2node/v2node server
Restart=always
RestartSec=10
...
EOF
    ...
    if [[ ! -f /etc/m2node/config.json ]]; then
        ...
        if [[ -n "$API_HOST_ARG" && -n "$NODE_ID_ARG" && -n "$API_KEY_ARG" ]]; then
            ...
            echo -e "${green}已根据参数生成 /etc/m2node/config.json${plain}"
            first_install=false
        else
            cp config.json /etc/m2node/
            first_install=true
        fi
    else
        if [[ x"${release}" == x"alpine" ]]; then
    ...
    curl -o /usr/bin/m2node -Ls https://raw.githubusercontent.com/clavin-dev/m2node/main/script/m2node.sh
    ...
    if [[ $first_install == true ]]; then
        read -rp "检测到你为第一次安装 v2node，是否自动生成 /etc/m2node/config.json？(y/n): " if_generate
